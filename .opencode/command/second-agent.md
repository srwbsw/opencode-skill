...
If `$REVIEW_SCRIPT` is empty or not executable, tell the user to install second-agent (the `second-agent-skill` plugin) in Claude Code or Codex (or set `SECOND_AGENT_REVIEW=/abs/path/to/review.js`), then stop.
