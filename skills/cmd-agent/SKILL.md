...
Use `agent.js` instead of `review.js` when the goal is to have Command Code **do** something — write tests, fix a bug, add a feature, refactor — rather than just comment on it. Same engine/model selection as above; different entry point, different safety model.
...
```bash
"$AGENT_SCRIPT" --engine=cmd --cwd=. --unrestricted "Add a CHANGELOG entry for this change, then run the test suite."
```
