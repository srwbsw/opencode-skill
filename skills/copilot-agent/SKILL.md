...
Copilot uses its default model unless you specify one — there's no listing command, so ask a single bundled question: default, or a specific model name? Pass it inline as `--engine=copilot:<model>`, or bare `--engine=copilot` for the default.
...
```bash
# Default model:
"$REVIEW_SCRIPT" --engine=copilot --cwd=<repo-path> [--diff=<spec>|--file=<path>] "<review prompt>"

# Specific model:
"$REVIEW_SCRIPT" --engine=copilot:<model> --cwd=<repo-path> [--diff=<spec>|--file=<path>] "<review prompt>"
```

## Reading the result

Stdout prints `ANSWER FILE: <path>`; the last line is a `SECOND_OPINION_RESULT: {...}` JSON. Read the ANSWER FILE with the Read tool for Copilot's clean answer.
No ANSWER FILE line? Read the LOG FILE path instead. Exit `3` means no usable answer — retry, or switch engines.

## Safety toggle

By default `review.js` adds `-s --plan --allow-all-tools --deny-tool=write`. Pass `--unrestricted` only when Copilot needs to edit files or run commands — `review.js` drops those flags, logged to stderr.

## Presenting results

Show the full response under `## Copilot's Take` (`(<model>)` if pinned). Don't filter or summarize — fix issues raised and note what changed.

## Task mode

Use `agent.js` instead of `review.js` when the goal is to have Copilot **do** something — write tests, fix a bug, add a feature, refactor — rather than just comment on it. Same engine/model selection as above; different entry point, different safety model.
...
```bash
"$AGENT_SCRIPT" --engine=copilot --cwd=. --unrestricted "Add a CHANGELOG entry for this change, then run the test suite."
```
