...
None needed. Gemini CLI picks its model automatically — always use bare `--engine=gemini` (no `:model` suffix).
...
```bash
"$REVIEW_SCRIPT" --engine=gemini --cwd=<repo-path> [--diff=<spec>|--file=<path>] "<review prompt>"
```

## Reading the result

Stdout prints `ANSWER FILE: <path>`; the last line is a `SECOND_OPINION_RESULT: {...}` JSON. Read the ANSWER FILE with the Read tool for Gemini's clean answer.
No ANSWER FILE line? Read the LOG FILE path instead. Exit `3` means no usable answer — retry, or switch engines.

## Safety toggle

By default `review.js` adds `-s --approval-mode plan`. Pass `--unrestricted` only when Gemini needs to edit files or run commands — `review.js` drops the flags, logged to stderr.

## Presenting results

Show the full response under `## Gemini's Take`. Don't filter or summarize — fix any issues raised and note what changed.

## Task mode

Use `agent.js` instead of `review.js` when the goal is to have Gemini **do** something — write tests, fix a bug, add a feature, refactor — rather than just comment on it. Same engine/model selection as above; different entry point, different safety model.
...
```bash
"$AGENT_SCRIPT" --engine=gemini --cwd=. --unrestricted "Add a CHANGELOG entry for this change, then run the test suite."
```
