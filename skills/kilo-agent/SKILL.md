...
Use `agent.js` instead of `review.js` when the goal is to have Kilo **do** something — write tests, fix a bug, add a feature, refactor — rather than just comment on it. Same provider/model selection as above (always ask, no default bypass); different entry point, different safety model.
...
```bash
"$AGENT_SCRIPT" --engine=kilo:<kilo/provider/model> --cwd=. --unrestricted "Add a CHANGELOG entry for this change, then run the test suite."
```
