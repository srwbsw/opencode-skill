...
Tests are plain Node scripts (no framework). Each prints `PASS/FAIL [name]` and exits non-zero on failure. Run one directly: `node test/spawn.test.js`. All four run via `pnpm run lint`.

Suites: `safety` (flag contract), `shell-quote`, `env-guard` (secret matcher), `spawn` (review.js integration). New test files must be added to the `lint` script in `package.json`.
...
- `FAKE_BEHAVIOR` — `ok` / `silent N` / `hang` / `probe N` / `empty` / `noenvelope` / `echo-stdin` (codex fixture)
